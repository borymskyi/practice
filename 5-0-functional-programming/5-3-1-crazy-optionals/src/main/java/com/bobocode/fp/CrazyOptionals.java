...
//        .of is thrown an exception, .ofNullable does not throw an exception when an "optional" has null.
...
//      orElse - if T already exists and needs to be passed, and orElseGet - if it needs to be generated
        return accountProvider.getAccount()
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream()
                ...
                .flatMap(streamOptionalBigDecimal -> streamOptionalBigDecimal.stream())
                .mapToDouble(bd -> bd.doubleValue())
                .reduce(0, (bd1, bd2) -> bd1 + bd2);
//              .reduce(0, Double::sum)
        //      .sum()
    }
}
