...
public class CrazyStreams {
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream()
                .collect(Collectors.partitioningBy(account ->
                        account.getSex().equals(Sex.MALE)));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream()
                .collect(Collectors.groupingBy(account ->
                        StringUtils.substringAfter(account.getEmail(), "@")));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream()
                .mapToInt(accounts ->
                        accounts.getFirstName().length() + accounts.getLastName().length())
                .sum();
    }
...
//        BigDecimal result = accounts.stream()
//                .map((account) -> {
...
//                .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    public Map<Long, Account> collectAccountsById() {
        return accounts.stream()
                .collect(Collectors.toMap(account -> account.getId(), a2 -> a2));
    }
...
//        Map<String, BigDecimal> result = accounts.stream()
//                .filter(new Predicate<Account>() {
//                    @Override
//                    public boolean test(Account account) {
//                        return account.getCreationDate().getYear() == year;
//                    }
//                })
...
//                            }
//                        },
//                        new Function<Account, BigDecimal>() {
//                            @Override
...
//        return result;
    ...
    return accounts.stream()
            .collect(
                    Collectors.groupingBy(a -> a.getBirthday().getMonth(), Collectors.mapping(
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream()
                .collect(Collectors.groupingBy(
                        ...
                        Collectors.mapping(Account::getBalance, Collectors.reducing(BigDecimal.ZERO, (bd1, bd2) -> bd1.add(bd2)))));
    }
